...
class PriorityFallbackReceiver implements ReceiverInterface
{
    /**
     ...
     * @var string[]
     */
    private array $fallbackSchedule;
    ...
    public function __construct(
        ...
        $this->fallbackSchedule = $this->buildSchedule($weights);
        $this->envelopeReceivers = new \SplObjectStorage();
    }

    public function get(): iterable
    {
        $primaryReceiver = $this->receivers[$this->primary] ?? null;
        if ($primaryReceiver) {
            foreach ($primaryReceiver->get() as $envelope) {
                $this->envelopeReceivers[$envelope] = $primaryReceiver;
                return [$envelope];
            }
        }

        $count = count($this->fallbackSchedule);
        if ($count === 0) {
            return [];
        }

        for ($i = 0; $i < $count; $i++) {
            $key = $this->fallbackSchedule[$this->cursor];
    ...
    private function buildSchedule(array $weights): array
    {
        ...
        $keys = array_values(array_filter(array_keys($this->receivers), function (string $key): bool {
